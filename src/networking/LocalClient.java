...
public class LocalClient extends Client implements Runnable {
    ...
    public static void connect() {
        utility.Console.message("Trying to connect LocalClient", instance);
        if (instance != null) {
            utility.Console.error("Failed to connect LocalClient. LocalClient is already connected!", instance);
            return;
        }
        ...
        try {
            ...
            utility.Console.error("Failed to connect LocalClient!", instance);
            ex.printStackTrace();
        }
        utility.Console.message("LocalClient connected to PORT : " + Server.PORT + " with address : " + IP_ADDER, instance);
    }
    ...
    public static void disconnect() {
        utility.Console.message("Trying to disconnect LocalClient", instance);
        if (instance == null) {
            utility.Console.warning("Failed to disconnect LocalClient. LocalClient is not connected!", null);
            return;
        }
        ...
        try {
            ...
            utility.Console.error("Failed to disconnect LocalClient!", instance);
            ex.printStackTrace();
        }
        utility.Console.message("LocalClient disconnected", instance);
    }
    ...
    public void run() {
        if (getSocket().isClosed()) {
            utility.Console.error("LocalClient Socket is closed!", this);
            return;
        }

        utility.Console.message("LocalClient started listening", this);
        do {
            this.packet = receive();
        } while (handlePacket());
        utility.Console.message("LocalClient stopped listening", this);
    }
